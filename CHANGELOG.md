...
- Fix: `Orchestrator.orchestrate_gap_seed` now proceeds to generate seeds and run the crawl even when an explicit entity record is missing (we now gather suggested domains from document types); added `tests/test_orchestrator_integration.py` to validate the end-to-end gap→seed→crawl flow.
- Added `probe investigate <query>` CLI: integrates `GapDetector` → `SeedGenerator` → `Orchestrator`, supports dry-run and JSON output, and includes tests and docs.
