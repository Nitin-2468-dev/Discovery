...
class Metrics:
    def __init__(self):
        ...
        try:
            ...
            self.fetch_duration = Histogram("probe_fetch_duration_seconds", "Fetch duration seconds")
            self.fetch_backoff = Histogram("probe_fetch_backoff_seconds", "Backoff/wait seconds")
            self._enabled = True
        except Exception:
