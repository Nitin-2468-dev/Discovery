...
def insert_domain(m, domain_name, pages, docs, yield_score=0.5, trust_score=0.5, last=None):
    last = last or datetime.now(timezone.utc).isoformat()
    m.conn.execute("INSERT INTO domains (domain_name, pages_crawled, documents_found, yield_score, trust_score, last_crawled_at) VALUES (?,?,?,?,?,?)", (domain_name, pages, docs, yield_score, trust_score, last))
...
def test_normalized_count_values(tmp_path):
    ...
    out_none = gd_none.analyze_entity_gaps("EN", ["manual", "datasheet"], include_scores=True)
    ...
    out_per = gd_per.analyze_entity_gaps("EN", ["manual", "datasheet"], include_scores=True)
    ...
    out_log = gd_log.analyze_entity_gaps("EN", ["manual", "datasheet"], include_scores=True)
    ...
    out_ppl = gd_ppl.analyze_entity_gaps("EN", ["manual", "datasheet"], include_scores=True)
    ds_ppl = {d['domain']: d for d in out_ppl['domain_scores']}
    assert isclose(ds_ppl['a.example.com']['normalized_count'], math.log1p(100.0 / 10.0))
