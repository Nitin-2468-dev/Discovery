...
def test_weights_affect_scoring():
    class MapWithDomains:
        def get_domains_with_doc_type(self, doc_type, limit=5):
            return [
                types.SimpleNamespace(domain_name="a.example.com"),
                types.SimpleNamespace(domain_name="b.example.com"),
            ]
        ...
        def get_domain(self, name):
            ...
            return types.SimpleNamespace(
                domain_name=name, yield_score=0.9, trust_score=0.9, last_crawled_at=None
            )
    ...
    gd_weighted = GapDetector(
        m, weights={"count": 10.0, "yield": 0.0, "trust": 0.0, "recent": 0.0}
    )
