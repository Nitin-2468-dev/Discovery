...
def test_fetcher_honors_flag_to_ignore_retry_after(monkeypatch):
    ...
    res = fetch("https://example.com/x", max_retries=1, backoff_factor=0.5, honor_retry_after=False, sleep_func=fake_sleep)
    ...
    calls.clear()
    sleeps.clear()
    res = fetch("https://example.com/x", max_retries=1, backoff_factor=0.5, honor_retry_after=True, sleep_func=fake_sleep)
    assert res.get('status_code') == 200
    assert len(sleeps) == 1
    assert sleeps[0] >= 4.5
