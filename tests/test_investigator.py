...
def test_investigator_runs_fetches_when_not_dry(tmp_path, monkeypatch):
    ...
    def fake_fetch(url, timeout=5, max_retries=1, backoff_factor=0.0, min_delay=0.0):
        return {"status_code": 200, "content_type": "text/html; charset=utf-8", "error": None, "raw_bytes": b"<html></html>", "title": "T", "links": []}
    ...
    monkeypatch.setattr(ingestmod, 'ingest_fetch_result', lambda m_obj, r: {'ingested': True})
