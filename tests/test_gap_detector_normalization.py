...
def test_normalization_per_page_prefers_low_pages_high_density(tmp_path):
    ...
    m.conn.execute(
        ...
        ("a.example.com", 200, 100, 0.2, 0.5, now),
    )
    for i in range(100):
        m.add_document(
            Document(
...
def test_normalization_log_reduces_skew(tmp_path):
    ...
    m.conn.execute(
        "INSERT INTO domains (domain_name, pages_crawled, documents_found, yield_score, trust_score, last_crawled_at) VALUES (?,?,?,?,?,?)",
        ("x.example.com", 10, 1000, 0.4, 0.4, now),
    )
    for i in range(1000):
        m.add_document(
            Document(
    ...
    m.conn.execute(
        "INSERT INTO domains (domain_name, pages_crawled, documents_found, yield_score, trust_score, last_crawled_at) VALUES (?,?,?,?,?,?)",
        ("y.example.com", 10, 40, 0.9, 0.9, now),
    )
    for i in range(40):
        m.add_document(
            Document(
    ...
    gd_raw = GapDetector(
        m,
        normalize="none",
        weights={"count": 2.0, "yield": 1.0, "trust": 0.5, "recent": 0.5},
    )
    out_raw = gd_raw.analyze_entity_gaps(
        "E2", ["manual", "datasheet"], include_scores=True
    )

    gd_log = GapDetector(
        m,
        normalize="log",
        weights={"count": 2.0, "yield": 1.0, "trust": 2.0, "recent": 0.5},
    )
    ...
    gap_log = log_scores.get("x.example.com", 0.0) - log_scores.get(
        "y.example.com", 0.0
    )
    assert (
        gap_log < gap_raw
    ), f"Expected log normalization to reduce score gap, got raw {gap_raw} vs log {gap_log}"
