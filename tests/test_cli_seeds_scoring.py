...
def test_seeds_run_score_and_persist(monkeypatch, tmp_path):
    ...
    def handler(request):
        return httpx.Response(200, headers={"content-type": "text/html; charset=utf-8"}, content=b"<html><head><title>One</title></head><body><p>maintenance manual</p><a href=\"/a\">A</a></body></html>")
    ...
    runner = CliRunner()
    res = runner.invoke(cli, ["seeds", "run", str(seeds), "--limit", "1", "--score", "--persist-scores", "--db", db, "--summary-csv", str(csvp)])
...
def test_seeds_run_score_to_csv_only(monkeypatch, tmp_path):
    ...
    def handler(request):
        return httpx.Response(200, headers={"content-type": "text/html; charset=utf-8"}, content=b"<html><head><title>Two</title></head><body><p>manual</p></body></html>")
    ...
    runner = CliRunner()
    res = runner.invoke(cli, ["seeds", "run", str(seeds), "--limit", "1", "--score", "--summary-csv", str(csvp)])
    assert res.exit_code == 0
    import csv
    with open(csvp, newline='', encoding='utf-8') as f:
        r = csv.DictReader(f)
