...
def test_include_scores_returns_sorted_domain_scores(tmp_path):
    ...
    m.conn.execute(
        ...
        ("d1.example.com", 10, 50, 0.2, 0.4, now),
    )
    m.conn.execute(
        "INSERT INTO domains (domain_name, pages_crawled, documents_found, yield_score, trust_score, last_crawled_at) VALUES (?,?,?,?,?,?)",
        ("d2.example.com", 5, 10, 0.9, 0.9, now),
    )
    for i in range(10):
        m.add_document(
            Document(
...
def test_malformed_last_crawled_does_not_raise_and_sets_recent_zero(tmp_path):
    ...
    m.conn.execute(
        "INSERT INTO domains (domain_name, pages_crawled, documents_found, yield_score, trust_score, last_crawled_at) VALUES (?,?,?,?,?,?)",
        ("m.example.com", 10, 10, 0.5, 0.5, "not-a-date"),
    )
    for i in range(10):
        m.add_document(
            Document(
...
def test_per_page_log_reduces_skew(tmp_path):
    ...
    m.conn.execute(
        "INSERT INTO domains (domain_name, pages_crawled, documents_found, yield_score, trust_score, last_crawled_at) VALUES (?,?,?,?,?,?)",
        ("x.example.com", 100, 1000, 0.4, 0.4, now),
    )
    for i in range(1000):
        m.add_document(
            Document(
    ...
    m.conn.execute(
        "INSERT INTO domains (domain_name, pages_crawled, documents_found, yield_score, trust_score, last_crawled_at) VALUES (?,?,?,?,?,?)",
        ("y.example.com", 5, 40, 0.9, 0.9, now),
    )
    for i in range(40):
        m.add_document(
            Document(
    ...
    gd_none = GapDetector(
        m,
        normalize="none",
        weights={"count": 2.0, "yield": 1.0, "trust": 0.5, "recent": 0.0},
    )
    out_none = gd_none.analyze_entity_gaps(
        "E-PL", ["manual", "datasheet"], include_scores=True
    )

    gd_per_page_log = GapDetector(
        m,
        normalize="per_page_log",
        weights={"count": 2.0, "yield": 1.0, "trust": 2.0, "recent": 0.0},
    )
    out_ppl = gd_per_page_log.analyze_entity_gaps(
        "E-PL", ["manual", "datasheet"], include_scores=True
    )
    ...
    gap_raw = raw_scores.get("x.example.com", 0.0) - raw_scores.get(
        "y.example.com", 0.0
    )
    gap_ppl = ppl_scores.get("x.example.com", 0.0) - ppl_scores.get(
        "y.example.com", 0.0
    )
