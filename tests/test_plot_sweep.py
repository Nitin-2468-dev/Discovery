...
def test_expand_row_to_points_json():
    row = {
        "domain_scores_json": json.dumps([{"domain": "a.com", "composite_score": 0.5}]),
        "weight_count": "3",
    }
    pts = list(plot_sweep._expand_row_to_points(row, None))
    ...
    rows = [
        {"domain": "example.com", "composite_score": "1.0", "weight_count": "1"},
        {
            "domain_scores_json": json.dumps(
                [{"domain": "example.com", "composite_score": 2.0}]
            ),
            "weight_count": "2",
        },
    ]
...
def test_aggregate_heatmap(tmp_path):
    ...
    xs, ys, z = plot_sweep._aggregate_heatmap(
        csv_path, "example.com", "weight_count", "weight_yield", None
    )
    assert xs == [1.0, 2.0]
    assert ys == [0.1, 0.2]
    assert len(z) == 2
