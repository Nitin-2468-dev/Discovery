...
def test_tiebreak_is_deterministic_by_domain_name(tmp_path):
    ...
    m.conn.execute("INSERT INTO domains (domain_name, pages_crawled, documents_found, yield_score, trust_score, last_crawled_at) VALUES (?,?,?,?,?,?)", ("b.example.com", 10, 5, 0.5, 0.5, now))
    m.conn.execute("INSERT INTO domains (domain_name, pages_crawled, documents_found, yield_score, trust_score, last_crawled_at) VALUES (?,?,?,?,?,?)", ("a.example.com", 10, 5, 0.5, 0.5, now))

    for i in range(5):
        m.add_document(Document(id=None, title=f"A{i}", doc_type="datasheet", hash=f"a{i}", url=f"https://a/{i}.pdf", domain="a.example.com"))
        m.add_document(Document(id=None, title=f"B{i}", doc_type="datasheet", hash=f"b{i}", url=f"https://b/{i}.pdf", domain="b.example.com"))
    ...
    assert [d['domain'] for d in out['domain_scores']] == out['suggested_domains'][: len(out['domain_scores'])]
