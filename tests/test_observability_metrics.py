import types
from probe import observability
...
def test_fetcher_emits_metrics(monkeypatch):
    ...
    res = fetch('https://example.com/test', max_retries=0, backoff_factor=0.1)
    ...
    assert any(c for c in rec.calls if c[0] == 'obs' and c[1] == 'fetch_duration_seconds')
