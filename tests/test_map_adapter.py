...
def test_map_adapter_basic(tmp_path: Path):
    ...
    # add a document
    doc = Document(id=None, title="Doc", doc_type="pdf", hash="h1", url="http://example.local/doc.pdf", domain="example.local")
...
def test_map_adapter_preserves_metadata(tmp_path: Path):
    ...
    page = Page(id=None, url="http://example.local/withmeta", domain="example.local", metadata=metadata)
    pid = adapter.add_page(page)
    assert pid >= 0
