...
def setup_two_domain_scenario(tmp_path):
    ...
    m.add_document(Document(id=None, title="A1", doc_type="datasheet", hash="ad1", url="https://a/1.pdf", domain="a.example.com"))
    m.add_document(Document(id=None, title="A2", doc_type="datasheet", hash="ad2", url="https://a/2.pdf", domain="a.example.com"))

    # Domain B: lower count, higher yield + recency
    m.conn.execute("INSERT INTO domains (domain_name, pages_crawled, documents_found, yield_score, trust_score, last_crawled_at) VALUES (?, ?, ?, ?, ?, ?)", ("b.example.com", 5, 3, 0.95, 0.9, now))
    # Add one datasheet to domain B
    m.add_document(Document(id=None, title="B1", doc_type="datasheet", hash="bd1", url="https://b/1.pdf", domain="b.example.com"))
...
def test_weight_configuration_changes_ordering(tmp_path):
    ...
    gd_yield = GapDetector(m, weights={"count": 0.2, "yield": 3.0, "trust": 0.5, "recent": 1.0})
